...
then begin
ShowMessage('Укажите тип подключения!');
exit;
end;
if (server.Checked=True) and  ( client.Checked=True )
then begin
ShowMessage('Укажите один тип подключения!');
exit;
end;
if nikname.Text='' then begin
ShowMessage('Укажите свой никнейм!');
exit;
end;
...
if Form1.Caption='Быстрая ЭЦП на основе алгоритма Фиата-Шамира' then
ServerSocket1.Port:=65003;
  ...
  ServerSocket1.Open; //запускаем
   ...
   sLabel1.Caption:= 'Ваш IP: '+GetLocalIP;
   //sPanel1.Caption:='Ожидается подключение клиента...';
   con.Caption:='Ожидание клиента...' ;
   sButton8.Visible:=true;
   end;
if ( client.Checked=True ) then begin
if   sEdit1.Text='' then begin
 ShowMessage('Укажите IP-адрес сервера!');
exit;
end;
  Form1.Left:=690; Form1.Top:=0;
  server.Enabled:=false; client.Enabled:=false;
  ClientSocket1.Address:=sEdit1.Text; // IP адрес сервера
if Form1.Caption='Электронная цифровая подпись El Gamal' then
...
begin
  ...
  SendMes('%');
  ServerSocket1.Close;
  //ServerSocket1.Active:=False; // разактивируем сервер
  end;
  if ( ClientSocket1.Active=true ) then begin
  ClientSocket1.Close;
  //ClientSocket1.Active:=False; //разактивируем клиента
